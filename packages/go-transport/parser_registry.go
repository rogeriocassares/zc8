...
// NewParserRegistry creates a new registry.
...
// LoadFromDatabase queries transport_parser table and instantiates parsers.
func (pr *ParserRegistry) LoadFromDatabase(ctx context.Context) error {
	...
	query := `
		SELECT id, code, parser_type, is_active
		FROM transport_parser
		WHERE is_active = true
		ORDER BY parser_type, code
	`

	rows, err := pr.db.QueryContext(ctx, query)
	if err != nil {
		return fmt.Errorf("failed to query transport_parser: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var id int64
		var code, parserType string
		var isActive bool

		if err := rows.Scan(&id, &code, &parserType, &isActive); err != nil {
			pr.logger.Printf("Error scanning parser row: %v", err)
			continue
		}
		...
		case "gateway":
			if err := pr.instantiateGatewayParser(code); err != nil {
				pr.logger.Printf("Failed to load %s gateway parser: %v", code, err)
				continue
			}
			gatewayCount++
		case "device":
	...
	if err = rows.Err(); err != nil {
		return fmt.Errorf("error reading parser rows: %w", err)
	}
